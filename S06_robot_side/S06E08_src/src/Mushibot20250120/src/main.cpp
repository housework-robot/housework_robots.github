    ...
    size_t length) 
{
    ...
    switch (type) {
        ...
        case WStype_DISCONNECTED:
            Log.noticeln("WebSocket Client '%u' is disconnected.\n", num);
            break;

        // New client has connected
        case WStype_CONNECTED:    
            {
                Log.noticeln("WebSocket Client '%u' is connected.\n", num);
            }
            break;
        ...
        case WStype_BIN:
        case WStype_ERROR:
            Log.noticeln("WS-WebSocket Client: '%u', WStype_ERROR, Payload: '%s'.\n", num, payload);
...
void handle_wifiConnected(WiFiEvent_t event, WiFiEventInfo_t info){
    Log.noticeln("The mushibot is connected to wifi network '%s', waiting for getting IP address.\n", 
        String(WiFi.SSID()) );
}

void handle_wifiGotIP(WiFiEvent_t event, WiFiEventInfo_t info){
    Log.noticeln("The mushibot is connected to '%s', with '%s' IP address.\n", 
        String(WiFi.SSID()), WiFi.localIP().toString());
}

void handle_wifiDisconnected(WiFiEvent_t event, WiFiEventInfo_t info){
    Log.noticeln("The mushibot is disconnected from wifi network.\n" );
