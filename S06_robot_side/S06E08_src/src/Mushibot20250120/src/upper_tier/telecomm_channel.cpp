...
void TelecommChannel::scan_wifi() {
    ...
    if (num_wifi == 0) {
        Log.warningln(F("No wifi network is found.\n"));
    } 
    else {  
        for (int i = 0; i < num_wifi; ++i) {
            // wifi_auth = String((WiFi.encryptionType(i) == WIFI_AUTH_OPEN) ? " without password" : " with password");
            Log.traceln(" [%d] Wifi-name: '%s', Signal-strength: %d, Authentication: %s",
...
String TelecommChannel::connect_wifi(String ssid, String password)
{
    ...
    if (cnt > 5) {
        Log.warningln("connect_wifi(): The mushibot can NOT connected to '%s' wifi network.\n", 
            String(WiFi.SSID()));
    } 
    else {
        local_ip = WiFi.localIP().toString();
        Log.traceln("connect_wifi(): The mushibot is connected to '%s' wifi network, with '%s' local IP address.\n", 
...
IPAddress TelecommChannel::get_public_IP_address() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpResponseCode > 0) {            
            ...
            Log.traceln("The Mushibot's public IP address is: '%s'.\n", public_ip_str.c_str());
            ...
            Log.warningln("Cannot access '%s' to get public IP address, http code is: '%d'.\n", 
...
String TelecommChannel::http_get(String http_url) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (http_res_code == HTTP_CODE_OK) {            
            http_res_str = http_client.getString();
            Log.verboseln("The following content is retrieved from '%s' by 'HTTP GET':", http_url.c_str());
            Log.verboseln(http_res_str.c_str());
            Log.verboseln("The above content is retrieved from '%s' by 'HTTP GET'.\n", http_url.c_str());
        }
        else {
            Log.warningln("Cannot access '%s' for http GET, http code is: '%d'.", 
                http_url.c_str(), http_res_code);
        }
    ...
    if (secure_client) {
        ...
        if (http_client.begin(*secure_client, https_url)) { 
            ...
            if (httpCode > 0) {
                ...
                Log.verboseln("GET returns code: %d", httpCode);
        
                // file found at server
                if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
                    // print server response payload
                    https_res_str = http_client.getString();
                    Log.verboseln("The following is the content retrieved from '%s' by 'HTTPS GET':", https_url.c_str());
                    Log.verboseln(https_res_str.c_str());
                    Log.verboseln("The above content is retrieved from '%s'  by 'HTTPS GET'.\n", https_url.c_str());
                }
            }
            else {
                Log.warningln("GET '%s' failed, error: %s", 
                    https_url.c_str(),
        ...
        Log.warningln("Unable to create NetworkClientSecure instance.\n");
    }
